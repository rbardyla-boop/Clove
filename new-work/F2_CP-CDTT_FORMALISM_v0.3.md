...
- S: PASS — roles and relation map.
- F: PASS — target equation is well-formed and dimensionally valid under target units.
- M: can PASS only with target-specific molecular transport mechanism/evidence; it is not inherited from heat conduction.
- P/V: require target-specific predictions/tests.
...
This can pass S as a limited relational analogy.
...
Therefore the analogy may be pedagogically useful at S but is scientifically illegal as an M mechanistic claim.
...
Weak-field general relativity admits gravitoelectromagnetic-like mathematical forms in certain gauges and limits. That can support a bounded F formal analogy.
...
The v0.3 formalism was replayed against the 20-case hand-built corpus at the schema/rule level.
...
- mixed cases can be represented by a qualification profile rather than forced into binary valid/invalid labels.
