...
test(`can-wait change survives normal-use check with coarse state and no network (${engine})`,async t=>{
  const {server,requests,origin}=await startServer();t.after(()=>new Promise(r=>server.close(r)));const page=await isolatedPage(t);await page.goto(`${origin}/digital-stewardship-03.html`);await changed(page);await choose(page,'I MISSED NOTHING IMPORTANT');
...
test(`important miss restores previous setting (${engine})`,async t=>{
  const {server,origin}=await startServer();t.after(()=>new Promise(r=>server.close(r)));const page=await isolatedPage(t);await page.goto(`${origin}/digital-stewardship-03.html`);await changed(page,'SOCIAL ACTIVITY');await choose(page,'I MISSED SOMETHING IMPORTANT');
...
test(`uncertain check restores rather than escalating (${engine})`,async t=>{
  const {server,origin}=await startServer();t.after(()=>new Promise(r=>server.close(r)));const page=await isolatedPage(t);await page.goto(`${origin}/digital-stewardship-03.html`);await changed(page,'NEWS / ENTERTAINMENT');await choose(page,"I'M NOT SURE");await choose(page,'RESTORED PREVIOUS SETTING');
  assert.equal(await page.getByRole('heading',{name:'CHECK COMPLETE'}).isVisible(),true);
});
...
test(`required, unclear, unknown class, and optional-no-change never force a change (${engine})`,async t=>{
  ...
  const cases=[['MARKETING / PROMOTIONAL','YES — I NEED IT NOW',null],['SOCIAL ACTIVITY',"I'M NOT SURE",null],['OTHER / NOT SURE','NO — IT CAN WAIT',null],['NONURGENT SHOPPING','NO — IT CAN WAIT','I DECIDED NOT TO CHANGE IT']];
  for(const [kind,intent,noChange] of cases){const context=await browser.newContext();const page=await context.newPage();await page.goto(`${origin}/digital-stewardship-03.html`);await enter(page,kind);await choose(page,intent);if(noChange)await choose(page,noChange);assert.equal(await page.getByRole('heading',{name:'CHECK COMPLETE'}).isVisible(),true);assert.equal(await page.getByRole('button',{name:'I CHANGED THIS ONE STREAM'}).count(),0);await context.close();}
});
...
test(`clear/reload and back-forward preserve only coarse progress (${engine})`,async t=>{
  const {server,origin}=await startServer();t.after(()=>new Promise(r=>server.close(r)));const page=await isolatedPage(t);await page.goto(`${origin}/before`);await page.goto(`${origin}/digital-stewardship-03.html`);await choose(page,'I HAVE ONE');await choose(page,'MARKETING / PROMOTIONAL');
  await page.evaluate(()=>{setTimeout(()=>history.back(),0);return true;});await page.waitForURL(/\/before$/,{timeout:10000});
  await page.evaluate(()=>{setTimeout(()=>history.forward(),0);return true;});await page.waitForURL(/\/digital-stewardship-03\.html$/,{timeout:10000});
  assert.match(await page.locator('#question').innerText(),/interrupt/i);await page.evaluate(k=>localStorage.removeItem(k),KEY);await page.reload();assert.equal(await page.getByRole('button',{name:'I HAVE ONE'}).isVisible(),true);
});
