#!/usr/bin/env bash
# Security regression runner — crisis-key local-storage hygiene (ADR-052).
# Serves the repo root over a local static server (so od-core.js / scripts resolve offline)
# and runs the crisis-key spec. Requires Playwright: set PW_REQUIRE_BASE to a node_modules parent.
set -uo pipefail
...
sleep 1
set +e
node "$ROOT/tests/security/crisis-key-hygiene.spec.mjs"
CODE=$?
set -e
