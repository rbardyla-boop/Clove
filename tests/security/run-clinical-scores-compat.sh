#!/usr/bin/env bash
# Security/compatibility regression runner — Gate 1: od_clinical_scores warm-cache
# compatibility, plaintext only (ADR-052). Serves the repo root over a local static
# server (so od-core.js / scripts resolve offline) and runs the clinical-scores-compat
# spec. Requires Playwright: set PW_REQUIRE_BASE to a node_modules parent.
set -uo pipefail
...
sleep 1
set +e
node "$ROOT/tests/security/clinical-scores-compat.spec.mjs"
CODE=$?
set -e
