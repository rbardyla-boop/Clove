#!/usr/bin/env bash
# Gate 3 — od_clinical_scores PRODUCTION ENCRYPTION regression runner (ADR-052).
# Serves the repo root over a local static server and runs the encrypted-mode spec
# directly against the shipped od-core.js/system-check.html — no patching, the
# ENCRYPT_KEYS flip is real. Requires Playwright: set PW_REQUIRE_BASE to a
# node_modules parent.
set -uo pipefail
...
sleep 1
set +e
node "$ROOT/tests/security/clinical-scores-encrypted.spec.mjs"
CODE=$?
set -e
