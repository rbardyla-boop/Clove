#!/usr/bin/env bash
# Security regression runner — cfhs-analyzer.html file-import XSS (ADR-052).
# Serves the repo root over a local static server (so /scripts/pdf.min.js etc. resolve offline) and
# runs the cfhs XSS spec. No Worker/network needed — the analyzer is fully local.
# Requires Playwright: set PW_REQUIRE_BASE to a node_modules parent if not installed locally.
set -uo pipefail
...
sleep 1
set +e
node "$ROOT/tests/security/cfhs-analyzer-xss.spec.mjs"
CODE=$?
set -e
