#!/usr/bin/env bash
# Phase 3E — run the mobile / public-playtest hardening checks against the local dev
# shim (same transport as run-two-client.sh). Requires a Playwright install (set
...
sleep 1
set +e
BASE_URL="http://127.0.0.1:$PORT_HTTP" WS_URL="ws://127.0.0.1:$PORT_WS/arcade/ws" node "$ROOT/tests/arcade/mobile-playtest.spec.mjs"
CODE=$?
set -e
