#!/usr/bin/env bash
# Runs the Phase 4B city-authority browser smoke against the local city dev shim.
...
# 1. static file server (serves .mjs/.js as JS) — same pattern as run-city-block.sh
node -e '
...
sleep 1
set +e
BASE_URL="http://127.0.0.1:$PORT_HTTP" WS_URL="ws://127.0.0.1:$PORT_WS/arcade/city/ws" node "$ROOT/tests/arcade/city-authority.spec.mjs"
CODE=$?
set -e
