#!/usr/bin/env bash
# Runs the Phase 2b room-admin (reset + status) browser validation against the dev
# shim. The dev shim is started WITH admin enabled + an EPHEMERAL admin token
# (generated here, never committed). The spec receives the same token via env.
set -euo pipefail
...
sleep 1
set +e
ADMIN_TEST_TOKEN="$TOKEN" BASE_URL="http://127.0.0.1:$PORT_HTTP" WS_URL="ws://127.0.0.1:$PORT_WS/arcade/ws" node "$ROOT/tests/arcade/room-admin.spec.mjs"
CODE=$?
set -e
