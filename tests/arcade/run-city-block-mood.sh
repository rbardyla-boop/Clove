#!/usr/bin/env bash
# Runs the Phase W-5 block mood browser smoke against the local city dev shim.
...
sleep 1
set +e
BASE_URL="http://127.0.0.1:$PORT_HTTP" WS_URL="ws://127.0.0.1:$PORT_WS/arcade/city/ws" node "$ROOT/tests/arcade/city-block-mood.spec.mjs"
CODE=$?
set -e
