#!/usr/bin/env bash
# Runs the cabinet-loop mobile city smoke (walk→see→enter→play→return) against the local city dev shim.
...
' "$ROOT" &
HTTP_PID=$!
# 2. city dev shim (reuses the SAME pure authority + event log + scheduler + host rank + stewardship + trial + district as the DO)
PORT=$PORT_WS node "$ROOT/workers/arcade/city-dev-shim.mjs" &
SHIM_PID=$!
sleep 1
set +e
BASE_URL="http://127.0.0.1:$PORT_HTTP" WS_URL="ws://127.0.0.1:$PORT_WS/arcade/city/ws" node "$ROOT/tests/arcade/city-loop-mobile.spec.mjs"
CODE=$?
set -e
