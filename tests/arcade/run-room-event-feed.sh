#!/usr/bin/env bash
# Runs the Phase 2f live room-event-feed browser validation against the dev shim. The
# shim shares the SAME pure room-events transition engine the Worker/DO use, and honours
# the TEST-ONLY __test_set_event_now clock (dev-gated on the real DO), so feed start/end/
# featured announcements are deterministic. Requires a Playwright install (set
# PW_REQUIRE_BASE to its node_modules parent if not project-local).
set -euo pipefail
...
sleep 1
set +e
BASE_URL="http://127.0.0.1:$PORT_HTTP" WS_URL="ws://127.0.0.1:$PORT_WS/arcade/ws" node "$ROOT/tests/arcade/room-event-feed.spec.mjs"
CODE=$?
set -e
