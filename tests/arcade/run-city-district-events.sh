#!/usr/bin/env bash
# Runs the Phase 6A Scheduled District Events browser smoke against the local city dev shim.
# Phase 6A is CLIENT-ONLY (no server change); the shim reuses the SAME pure authority/presence/district.
...
' "$ROOT" &
HTTP_PID=$!
# 2. city dev shim (reuses the SAME pure authority + presence push + district as the DO; Phase 6A adds no server change)
PORT=$PORT_WS node "$ROOT/workers/arcade/city-dev-shim.mjs" &
SHIM_PID=$!
sleep 1
set +e
BASE_URL="http://127.0.0.1:$PORT_HTTP" WS_URL="ws://127.0.0.1:$PORT_WS/arcade/city/ws" node "$ROOT/tests/arcade/city-district-events.spec.mjs"
CODE=$?
set -e
