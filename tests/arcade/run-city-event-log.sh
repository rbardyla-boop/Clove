#!/usr/bin/env bash
# Runs the Phase 4C city event-log + in-place interior browser smoke against the local city dev shim.
...
' "$ROOT" &
HTTP_PID=$!
# 2. city dev shim (reuses the SAME pure city authority + event log as the CityRoom DO)
PORT=$PORT_WS node "$ROOT/workers/arcade/city-dev-shim.mjs" &
SHIM_PID=$!
sleep 1
set +e
BASE_URL="http://127.0.0.1:$PORT_HTTP" WS_URL="ws://127.0.0.1:$PORT_WS/arcade/city/ws" node "$ROOT/tests/arcade/city-event-log.spec.mjs"
CODE=$?
set -e
