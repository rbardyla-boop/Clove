#!/usr/bin/env bash
# Runs the Phase 2d smart-lobby presence-UX browser validation against the dev shim.
# (Server is unchanged from Phase 2c; this exercises the client UI only.)
set -euo pipefail
...
sleep 1
set +e
BASE_URL="http://127.0.0.1:$PORT_HTTP" WS_URL="ws://127.0.0.1:$PORT_WS/arcade/ws" node "$ROOT/tests/arcade/room-presence-ux.spec.mjs"
CODE=$?
set -e
