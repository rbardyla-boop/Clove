#!/usr/bin/env bash
# Creator Foundation CF-3 — layered block editor browser smoke runner.
# Serves the repo root over a local static server and runs the layered editor spec. The editor is
# OFFLINE (no Worker, no shim, no network), so only a static file server is needed.
...
sleep 1
set +e
node "$ROOT/tests/creator/layered-editor.spec.mjs"
CODE=$?
set -e
