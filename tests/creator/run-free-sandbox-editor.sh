#!/usr/bin/env bash
# Creator Freedom v1 — Free Sandbox editor browser smoke runner.
# The builder is OFFLINE (no Worker, no shim); only a static file server is needed.
...
sleep 1
set +e
node "$ROOT/tests/creator/free-sandbox-editor.spec.mjs"
CODE=$?
set -e
