#!/usr/bin/env bash
# Creator Foundation CF-1 — local block editor browser smoke runner.
# Serves the repo root over a local static server and runs the editor spec. The editor is OFFLINE
# (no Worker, no shim, no network), so only a static file server is needed.
...
sleep 1
set +e
node "$ROOT/tests/creator/block-editor.spec.mjs"
CODE=$?
set -e
