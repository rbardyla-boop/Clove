#!/usr/bin/env bash
# Creator Foundation CF-4 — local arcade sandbox browser smoke runner.
# The sandbox is OFFLINE (no Worker, no shim); only a static file server is needed.
...
sleep 1
set +e
node "$ROOT/tests/creator/arcade-sandbox.spec.mjs"
CODE=$?
set -e
